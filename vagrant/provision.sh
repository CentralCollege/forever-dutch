...
echo "<VirtualHost *:80>
ServerName dev.foreverDutch.central.edu
...
ln -s /var/www/wordpress/foreverDutch.dev.conf /etc/apache2/sites-enabled/foreverDutch.dev.conf
...
echo "create database foreverDutch;" | mysql -u root
...
define('DB_PASSWORD', 'foreverDutch');
...
/var/www/wordpress/wp-cli option update blogdescription 'A test site for the foreverDutch.' \
	--path=/var/www/wordpress \
	--allow-root
...
file=/var/www/wordpress/wp-content/themes/foreverDutch/wp-import-data.xml
if [ -f $file ];
then
